...
const addToCart = async (req, res) => {
  try {
    ...
    // kiểm tra trước khi tăng quantity
    ...
    const populatedItems = cart.items.map(item => {
      ...
      return {
        ...
        image: item.variantImage || prod.image,
        totalStock: variant.sizes.find(s => s.size === item.size)?.stock || 0,
      };
    });
